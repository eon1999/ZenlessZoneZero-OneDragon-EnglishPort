...
class TeamSettingCard(MultiPushSettingCard):
    ...
    def __init__(self):
        ...
        MultiPushSettingCard.__init__(self, icon=FluentIcon.PEOPLE, title=gt('配队'), btn_list=self.btn_list)
...
class AutoBattleEditorInterface(VerticalScrollInterface):
    ...
    def init_left_part(self) -> QWidget:
        ...
        info_opt = HyperlinkCard(icon=FluentIcon.INFO, title=gt('当前仅用于信息展示'),
                                 content=gt('角色顺序 朱青妮 跟 青妮朱 是一样的'),
                                 text='', url='')
        ...
        basic_group = SettingCardGroup(gt('基础信息'))
        widget.add_widget(basic_group)
        self.author_opt = HyperlinkCard(icon=FluentIcon.PEOPLE, title=gt('作者'), text=gt('作者'),
                                        url=self.ctx.project_config.qq_link)
        self.author_opt.setContent(gt('制作不易 可以到作者主页点个赞'))
        basic_group.addSettingCard(self.author_opt)
        self.version_opt = HyperlinkCard(icon=FluentIcon.INFO, title=gt('版本'), text='1.0', url='')
        basic_group.addSettingCard(self.version_opt)
        self.introduction_opt = HyperlinkCard(icon=FluentIcon.INFO, title=gt('简介'), text='', url='')
        self.introduction_opt.linkButton.hide()
        basic_group.addSettingCard(self.introduction_opt)

        self.team_group = SettingCardGroup(gt('适用配队'))
    ...
    def update_display_by_config(self) -> None:
        ...
        if chosen:
            ...
            self.version_opt.setContent(
                f'{gt("感谢")} {self.chosen_config.thanks}' if self.chosen_config.thanks != '' else ''
            )
            self.introduction_opt.setContent(self.chosen_config.introduction)
        else:
            self.author_opt.linkButton.setText(gt('作者'))
