...
class ButtonGroup(SimpleCardWidget):
    ...
    def __init__(self, ctx: ZContext, parent=None):
        ...
        github_button = IconButton(
            ...
            tip_title=gt("GitHub仓库"),
            tip_content=gt("⭐点击收藏关注项目动态"),
            isTooltip=True,
        )
        ...
        shop_button = IconButton(
            ...
            tip_title=gt("🏅官方店铺???"),
            tip_content=gt("💵限时劲爆特惠仅需0元点击马上加入会员>>"),
            isTooltip=True,
        )
...
class HomeInterface(VerticalScrollInterface):
    ...
    def __init__(self, ctx: ZContext, parent=None):
        ...
        # 启动游戏按钮布局
        self.start_button = PrimaryPushButton(text=gt("启动一条龙🚀"))
    ...
    def _need_to_update_code(self, with_new: bool):
        if not with_new:
            self._show_info_bar(gt("代码已是最新版本"), "Enjoy it & have fun!")
            return
        else:
            self._show_info_bar(gt("有新版本啦"), gt("稍安勿躁~"))

    def _need_to_update_model(self, with_new: bool):
        if with_new:
            self._show_info_bar(gt("有新模型啦"), gt("到[设置-资源下载]更新吧~"), 5000)

    def _need_to_update_launcher(self, with_new: bool):
        if with_new:
            self._show_info_bar(gt("有新启动器啦"), gt("到[设置-资源下载]更新吧~"), 5000)
    ...
    def reload_banner(self, show_notification: bool = False) -> None:
        ...
        try:
            ...
            if show_notification:
                self._show_info_bar(gt("背景已更新"), gt("新的背景已成功应用"), 3000)
        except Exception as e:
