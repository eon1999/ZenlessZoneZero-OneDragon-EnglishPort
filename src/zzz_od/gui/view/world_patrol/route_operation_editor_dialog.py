...
class RouteOperationEditorDialog(QDialog):
    ...
    def _on_delete_clicked(self):
        ...
        if 0 <= current_row < len(self.current_op_list):
            reply = QMessageBox.question(self, gt('确认删除'),
                                       gt('确定要删除第 %d 个操作吗？') % current_row,
    ...
    def _on_save_clicked(self):
        ...
        for i, op in enumerate(self.current_op_list):
            if len(op.data) < 2:
                QMessageBox.warning(self, gt('数据错误'), gt('第 %d 个操作的数据不完整') % i)
                return
            try:
                # 验证坐标是否为数字
                float(op.data[0])
                ...
                QMessageBox.warning(self, gt('数据错误'), gt('第 %d 个操作的坐标数据必须是数字') % i)
                return
